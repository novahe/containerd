...
func (m *ShimTaskManager) Create(ctx context.Context, taskID string, bundle *Bundle, opts runtime.CreateOpts) (runtime.Task, error) {
	...
	t, err := func() (runtime.Task, error) {
		t, err := shimTask.Create(ctx, bundle.Path, opts)
		if err == nil || !errdefs.IsNotImplemented(err) {
			return t, err
		}

		// Creating a task can still hit an older task API when attaching to a running shim.
		...
		if ok {
			if derr := downgrader.Downgrade(); derr == nil {
				...
				return shimTask.Create(ctx, bundle.Path, opts)
			}
		}
		return t, err
	}()
...
func (m *ShimTaskManager) Load(ctx context.Context, bundle *Bundle) error {
	return m.shimManager.loadShim(ctx, bundle, func(instance ShimInstance) error {
		s, err := newLoadedShimTask(ctx, instance)
		if err != nil {
			return err
		}
		...
		if len(pInfo) == 0 || errors.Is(pidErr, errdefs.ErrNotFound) {
			...
			return cleanupLeakedTaskShim(ctx, s)
		}
...
func (m *ShimTaskManager) Delete(ctx context.Context, taskID string) (*runtime.Exit, error) {
	...
	exit, err := shimTask.delete(ctx, container.SandboxID != "", func(ctx context.Context, id string) {
...
func newLoadedShimTask(ctx context.Context, instance ShimInstance) (*shimTask, error) {
	...
	if _, err := s.PID(ctx); err != nil {
		...
		// Older running shims can still expose an older task API after restart.
		downgrader, ok := instance.(clientVersionDowngrader)
		if ok {
			if derr := downgrader.Downgrade(); derr == nil {
				log.G(ctx).WithError(err).WithField("id", instance.ID()).
...
func cleanupLeakedTaskShim(ctx context.Context, s *shimTask) error {
	dctx, cancel := timeout.WithContext(context.WithoutCancel(ctx), cleanupTimeout)
	defer cancel()

	if _, err := s.delete(dctx, false, func(context.Context, string) {}); err != nil && !errdefs.IsNotFound(err) {
		return err
	}
...
func (s *shimTask) Create(ctx context.Context, bundle string, opts runtime.CreateOpts) (runtime.Task, error) {
	if err := s.remoteTask.Create(ctx, bundle, opts); err != nil {
		return nil, err
	}

	if opts.RestoreFromPath {
		if err := s.restoreFromPath(ctx, opts.Checkpoint); err != nil {
			return nil, err
		}
...
func (s *shimTask) restoreFromPath(ctx context.Context, checkpointPath string) error {
	// Unpack rootfs-diff.tar if it exists.
	// This needs to happen between the Create() and Start() calls.
	rootfsDiff := filepath.Join(checkpointPath, "..", crmetadata.RootFsDiffTar)
	...
	if err != nil {
		return fmt.Errorf("failed to open rootfs-diff archive %s for import: %w", rootfsDiff, err)
	}
	...
	if err != nil {
		return fmt.Errorf("failed to decompress archive %s for import: %w", rootfsDiffTar.Name(), err)
	}

	rootfs := filepath.Join(s.Bundle(), "rootfs")
	if _, err := archive.Apply(ctx, rootfs, decompressed); err != nil {
		return fmt.Errorf("unpacking of rootfs-diff archive %s into %s failed: %w", rootfsDiffTar.Name(), rootfs, err)
	}
