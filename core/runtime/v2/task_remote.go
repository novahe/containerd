...
func (r *remoteTask) Create(ctx context.Context, bundle string, opts runtime.CreateOpts) error {
	...
	if opts.RestoreFromPath {
		if err := restoreRootfsFromCheckpoint(ctx, bundle, opts); err != nil {
			return err
		}
...
func restoreRootfsFromCheckpoint(ctx context.Context, bundle string, opts runtime.CreateOpts) error {
	// Unpack rootfs-diff.tar if it exists. This must happen after Create and before Start.
	...
	defer func() {
		if err := rootfsDiffTar.Close(); err != nil {
			log.G(ctx).Errorf("Unable to close file %s: %q", rootfsDiffTar.Name(), err)
		}
	}()
